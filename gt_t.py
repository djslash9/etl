...
if process_button:
    ...
    all_files_uploaded = (
        st.session_state.web_timeline and
        st.session_state.web_geomap_region and
        st.session_state.web_geomap_city and
        st.session_state.youtube_timeline and
        st.session_state.youtube_geomap_region and
    ...
    if not st.session_state.file_date:
        ...
        st.markdown('<div class="error-box"><strong>❌ Please upload all 6 CSV files.</strong></div>', unsafe_allow_html=True)
    else:
        with st.spinner("Processing all data..."):
            try:
                ...
                if df_web_timeline is not None and df_youtube_timeline is not None:
                    df_timeline_merged = pd.concat([df_web_timeline, df_youtube_timeline], ignore_index=True)
                ...
                if df_web_region is not None and df_youtube_region is not None:
                    df_region_merged = pd.concat([df_web_region, df_youtube_region], ignore_index=True)
                ...
                if df_web_city is not None and df_youtube_city is not None:
                    df_city_merged = pd.concat([df_web_city, df_youtube_city], ignore_index=True)
                    st.session_state.processed_data['city'] = df_city_merged
                
                st.markdown('<div class="success-box"><strong>✅ All data processed successfully! View results below.</strong></div>', unsafe_allow_html=True)
